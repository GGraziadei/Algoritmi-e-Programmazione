...
void  scambiaCorsaPointer(tabella_t *tab, int indice );
       ...
       if(tab->chiaveOrdinamento == start){
           ...
           if (idCorsa >= 0) {
               for (i = idCorsa;  i >= 0 && stampa && i < tab->n; i++) {
                   if (confrontaStr(tab->toCorsa[i]->start, search, strlen(search)) == 0)
                       stampaTratta(stdout, tab->toCorsa[i]);
                   else stampa = 0;
               }
           }
...
int linearSearch (tabella_t *tab, char *s){
    int i=0,conta=0;
    for(i=0; i<tab->n; i++){
        if( confrontaStr(tab->toCorsa[i]->start, s, strlen(s)) ==0) {
            stampaTratta(stdout, tab->toCorsa[i]);
            conta = conta+1;
        }
    }
...
int binarySearch (tabella_t *tab, char *s, int inf, int sup) {
    if(sup-inf == 0) {
        if (confrontaStr (tab->toCorsa[inf]->start, s, strlen(s) ) == 0)
            return inf;
        else
            return -1;
    }
    ...
    int m = (sup+inf)/2;
    if (confrontaStr (tab->toCorsa[m]->start, s, strlen(s)) >=0)
...
void Ordina (tabella_t *tab,e_key chiave){
    ...
    for(i=0; i<tab->n; i++){
        for(j=0;j<tab->n-i-1; j++){
            switch (chiave) {
                case time:
                    res = confrontaTime(tab->toCorsa[j], tab->toCorsa[j + 1]);
                    break;
                case codTratta :
                    res = confrontaStr(tab->toCorsa[j]->codTratta, tab->toCorsa[j + 1]->codTratta, 0);
                    break;
                case start :
                    res = confrontaStr(tab->toCorsa[j]->start, tab->toCorsa[j + 1]->start, 0);
                    break;
                case end :
                    res = confrontaStr(tab->toCorsa[j]->end, tab->toCorsa[j + 1]->end, 0);
                    break;
                default:
                    res = -1;
            }
            if(res>0){
                if(SHOW_SWAP){
                    stampaTratta(stdout,tab->toCorsa[j]);
                    stampaTratta(stdout,tab->toCorsa[j+1]);
                    printf("------------SCAMBIA-------------");
                }
                scambiaCorsaPointer(tab,j);
            }
...
void  scambiaCorsaPointer(tabella_t *tab, int indice ){
    Corsa *tmp = tab->toCorsa[indice];
    tab->toCorsa[indice]= tab->toCorsa[indice+1];
    tab->toCorsa[indice+1] = tmp;
    return;
}
...
void stampaFile (tabella_t *tab, char *fileout){
    ...
    int i;
    for(i=0; i<tab->n; i++)
        stampaTratta(out,tab->toCorsa[i]);
    fclose(out);
    return;
}
