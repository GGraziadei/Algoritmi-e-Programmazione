...
typedef struct {
    ...
    e_key last_update;
    /*Ho n=errore possibili ordinamenti quante sono le chiavi.
...
e_comandi wrapper_ricercaPartenza (tabella_t *tab){
    ...
    if(sscanf(cmd,"%s",search)==1 ){
       ...
       if(tab->ordinamenti[start].chiaveOrdinamento == start){
           printf("\nBINARY SEARCH\n");
           ...
           if (idCorsa >= 0) {
               for (i = idCorsa;  i >= 0 && stampa && i < tab->n; i++) {
                   if (confrontaStr(tab->ordinamenti[start].toCorsa[i]->start, search, strlen(search)) == 0)
                       stampaTratta(stdout, tab->ordinamenti[start].toCorsa[i]);
                   else stampa = 0;
               }
           }
...
int linearSearch (tabella_t *tab, char *s){
    int i=0,conta=0;
    for(i=0; i<tab->n; i++){
        if( confrontaStr(tab->ordinamenti[start].toCorsa[i]->start, s, strlen(s)) ==0) {
            stampaTratta(stdout, tab->ordinamenti[start].toCorsa[i]);
            conta = conta+1;
        }
    }
...
int binarySearch (tabella_t *tab, char *s, int inf, int sup) {
    if(sup-inf == 0) {
        if (confrontaStr (tab->ordinamenti[start].toCorsa[inf]->start, s, strlen(s) ) == 0)
            return inf;
        else
            return -1;
    }
    ...
    int m = (sup+inf)/2;
    if (confrontaStr (tab->ordinamenti[start].toCorsa[m]->start, s, strlen(s)) >=0)
...
void stampaSchermo (tabella_t *tab){
    int i=0;
    for(i=0; i<tab->n; i++)
        stampaTratta(stdout,tab->ordinamenti[tab->last_update].toCorsa[i]);
    return;
}
...
void stampaFile (tabella_t *tab, char *fileout){
    ...
    int i;
    for(i=0; i<tab->n; i++)
        stampaTratta(out,tab->ordinamenti[tab->last_update].toCorsa[i]);
    fclose(out);
    return;
}
