...
link_nodoPg linearSearchList (link_tabPg tabPg,PG_KEY id){
    ...
    while (x != NULL && !PG_KEYcompare(x->pg.id,id) )
        x = x->next;
    return x;
}
...
PG_KEY PG_KEYget(FILE *in,int init){
    ...
    do {
        ...
        assert(buf != NULL);
        toMaiuscolo(buf);
    } while (strlen(buf)!=6 || strncmp(buf,"PG",2)!=0);
    sscanf(buf,"PG%d",&k);
    return k;
}
