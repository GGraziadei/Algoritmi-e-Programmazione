...
typedef enum {
    r_stampa, r_ordina_time, r_ordina_tratta, r_ordina_partenza, r_ordina_arrivo,r_ricerca_partenza,r_ricerca_arrivo,r_ricerca_date,r_ricerca_codTratta, r_ritardo,r_ritardo_tot,r_date,r_fine,r_errore
}e_comandi;
...
typedef struct {
     ...
     * In stampa utilizzo l'ultimo update di ordinamento*/
} tabella_t;
 ...
 * 2.Funzionare anche in maniera non esatta, ovvero in caso di mancata corrispondeza ritornare l'indice che precede o segue un determinato valore*/

int main() {
    tabella_t tab;
    e_comandi CODcomando;
    int finito = 0;
    char c_tabella[r_errore][S]={ "stampa","ordina_time","ordina_tratta","ordina_partenza","ordina_arrivo","partenza","capolinea","ricerca_date","ricerca_tratta","ritardo","ritardo_tot","date","fine" };
    tab = leggiLog();
...
tabella_t leggiLog(){
    ...
    if(fscanf(fp,"%d",&tab.n)!=1)
        exit(-1);
    /*I file prensenti nel log non sono inizialmente ordinati */
    for(i=0; i<errore; i++)
...
e_comandi leggiComando(char c_tabella[][S]){
    ...
    char c_tabella_Comandi [r_errore][SS]={
            "1. Stampa log: stampa <nome_file/schermo> <chiave_ordinamento> \nATTENZIONE SE STAMPI SU FILE INSERISCI ESTENSIONE - Se non presente vettore ordinato stampa secondo last_update",
...
int binarySearch (tabella_t *tab, char *s, int inf, int sup,e_key chiave, e_searchType mod) {
    ...
    if(sup-inf == 0) {
        ...
        if(res <0 && mod == segue )
            ...
            * non esatte, utile la selezione di intervalli*/
...
IntervalloId selezionaIntervalloDate (char *d1,char*d2,tabella_t *tab, e_key chiave){
    ...
    /*Normalmente mi aspetto di avere un al più n-2 per poi controllare se l'intervallo esiste e abilitare l'intervallo.
     * Se ottengo n-1 ho sforato ed evito il controllo sup*/
