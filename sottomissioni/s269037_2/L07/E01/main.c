...
int power_set (tab_t *tab){
     ...
     * incremento di 1 la cardinalità del set di output. Count quindi non indica la cadinalità dell'insieme
     ...
     * la cardinalità dell'insieme delle parti di un set è pari a 2^(n=cardinalità_set)*/
    int k,count=0;
    for(k=0; k<=tab->num_n; k++){
...
int isVertexCover(tab_t *tab){
    ...
    /*Il vettore check mi permette di mantenere una complessità lineare per la isVertexCover(), utilizzando il metodo delle disposizioni lineari non sarebbe
     * necessaria tale allocazione in quanto la cardinalità di sol è sempre N ed avrà k elementi posti ad 1.
     * Ho scelto le combinazioni lineari per non compromettere "l'ordine di stampa" delle soluzioni per cardinalità*/
