...
link wrapper_insert_item (Item i, link head);
link liberaLista (link head);
...
link wrapper_add_item (e_add_mode mode,link head){
    /*Se già presente in struttura un user lo estraggo ed inserisco i dati aggiornati*/
    ...
    if (mode == s_schermo){
        user_x = item_gen_cmd();
        head =  wrapper_insert_item(user_x,head);
    }
    else if (mode == s_file){
        do {
...
link  wrapper_extract_code (link head){
    ...
    if(fscanf(stdin,"%s",code)==1) {
        if (code[0] > 'A')
            toMaiuscolo(code);
        if(linearSearch(head,code)!=NULL)
...
link wrapper_insert_item (Item i, link head){
    /*Se l'utente con dato codice è già presente in SD, allora interpreto come update la inserzione sullo stesso codice.

    if(head != NULL && linearSearch(head,i.id)!=NULL){
        printf("%s Presente in LISTA. Estraggo ed inserisco il nodo con item aggiornato",i.id);
        head = wrapper_extract(head,i.id);
    }*/
    printf("Insert/Update -> %s\n\n",i.id);
    return sort_ins(head,i);
}
link new_node (Item val,link next){
...
data_t str2date (char *str){
    data_t test;
    while (sscanf(str,"%d/%d/%d",&test.g,&test.m,&test.a) != 3 || test.g>31 || test.m>12 || test.a>TODAY_Y ){
...
Item item_gen_cmd(){
    /*le stringhe non contengono spazi, le acquisisco con fscanf()*/
    char tmp_str[S];
    int l=0;
    Item i;
    do{
